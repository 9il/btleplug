...
use objc2_core_bluetooth::{
    CBAdvertisementDataLocalNameKey, CBAdvertisementDataManufacturerDataKey, CBAdvertisementDataServiceDataKey,
    CBAdvertisementDataServiceUUIDsKey, CBAdvertisementDataTxPowerLevelKey, CBCentralManager, CBCentralManagerDelegate, CBCharacteristic,
    CBDescriptor, CBManagerState, CBPeripheral, CBPeripheralDelegate, CBService, CBUUID,
};
...
impl Debug for CentralDelegateEvent {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        match self {
            CentralDelegateEvent::DidUpdateState { state } => f.debug_struct("CentralDelegateEvent").field("state", state).finish(),
            CentralDelegateEvent::DiscoveredPeripheral {
                ...
                .finish(),
            CentralDelegateEvent::DiscoveredServices { peripheral_uuid, services } => f
            ...
            CentralDelegateEvent::DiscoveredCharacteristics {
                ...
                .field("characteristics", &characteristics.keys().collect::<Vec<_>>())
                .finish(),
            ...
            CentralDelegateEvent::ConnectedDevice { peripheral_uuid } => {
                f.debug_struct("ConnectedDevice").field("peripheral_uuid", peripheral_uuid).finish()
            }
            ...
            CentralDelegateEvent::TxPowerLevel {
                ...
                .finish(),
            CentralDelegateEvent::DidReadRssi { peripheral_uuid, rssi } => f
...
define_class!(
    // SAFETY: NSObject has no subclassing requirements; CentralDelegate does not impl Drop.
    #[unsafe(super(NSObject))]
    #[name = "BtlePlugCentralManagerDelegate"]
    ...
    unsafe impl CBCentralManagerDelegate for CentralDelegate {
        ...
        fn delegate_centralmanager_diddiscoverperipheral_advertisementdata_rssi(
            ...
            let advertisement_name = unsafe { adv_data.objectForKey(CBAdvertisementDataLocalNameKey) }
            ...
            let manufacturer_data = unsafe { adv_data.objectForKey(CBAdvertisementDataManufacturerDataKey) }
                .and_then(|d| d.downcast::<NSData>().ok());
            if let Some(manufacturer_data) = manufacturer_data {
                // SAFETY: advertisement payload is not mutated while we read it.
                let manufacturer_bytes = unsafe { manufacturer_data.as_bytes_unchecked() };

                if manufacturer_bytes.len() >= 2 {
                    let (manufacturer_id, manufacturer_data) =
                        manufacturer_bytes.split_at(2);
            ...
            let service_data = unsafe { adv_data.objectForKey(CBAdvertisementDataServiceDataKey) }
                .and_then(|d| d.downcast::<NSDictionary>().ok())
                .map(|d| unsafe { Retained::cast_unchecked::<NSDictionary<CBUUID, NSData>>(d) });
            if let Some(service_data) = service_data {
                let mut result = HashMap::new();
                for uuid in service_data.keys() {
                    if let Some(data) = service_data.objectForKey(&uuid) {
            ...
            let services = unsafe { adv_data.objectForKey(CBAdvertisementDataServiceUUIDsKey) }
                .and_then(|d| d.downcast::<NSArray>().ok())
                .map(|d| unsafe { Retained::cast_unchecked::<NSArray<CBUUID>>(d) });
            ...
            let tx_power_level = unsafe { adv_data.objectForKey(CBAdvertisementDataTxPowerLevelKey) }
                .and_then(|val| val.downcast::<NSNumber>().ok())
                .map(|val| val.as_i16());
...
impl std::fmt::Debug for CentralDelegate {
    fn fmt(&self, f: &mut Formatter) -> fmt::Result {
        f.debug_struct("CentralDelegate").finish_non_exhaustive()
    }
}
...
fn get_descriptor_value(descriptor: &CBDescriptor) -> Vec<u8> {
    ...
    let v = unsafe { descriptor.value() }.map(|value| {
        ...
        match clazz.name().to_bytes() {
            b"NSString" => value.downcast::<NSString>().map(|d| d.to_string().into_bytes()).unwrap_or_default(),
            b"NSData" => value.downcast::<NSData>().map(|d| d.to_vec()).unwrap_or_default(),
            b"NSNumber" => value
                .downcast::<NSNumber>()
                .map(|d| d.stringValue().to_string().into_bytes())
                .unwrap_or_default(),
            _ => {
